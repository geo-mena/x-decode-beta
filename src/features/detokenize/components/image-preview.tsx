...
interface ImagePreviewProps {
    data: DetokenizeResponseData | DetokenizeResponseData[] | null;
    error: string | null;
    isLoading: boolean;
...
export function ImagePreview({ data, error, isLoading, onDownload }: ImagePreviewProps) {
    ...
    const handleOpenOriginal = (imageData: DetokenizeResponseData) => {
    ...
    const handleCopyImage = async (imageData: DetokenizeResponseData) => {
        if (!imageData?.file_name) return;

        setCopyLoading(true);
        try {
            const img = new Image();
            const proxyUrl = detokenizeService.getProxyUrl(imageData.file_name);
    ...
    const tryFallbackCopyMethod = (
        ...
        imageData: DetokenizeResponseData
    ) => {
        try {
                ...
                <CardHeader className='flex flex-row items-center justify-between pb-2'>
                    ...
                    {activeImage && (
                        <div className='flex gap-2'>
                            ...
                            <ImageAnalyzer
                                data={activeImage}
                                //? onEnhanceApplied={handleEnhanceApplied}
                            />
                        </div>
                    )}
                </CardHeader>
