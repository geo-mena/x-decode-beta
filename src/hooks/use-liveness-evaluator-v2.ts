...
const VALID_MIME_TYPES = ['image/jpeg', 'image/jpg', 'image/png', 'image/bmp', 'image/gif', 'image/webp'] as const
...
export const useLivenessEvaluator = () => {
    ...
    const validateBase64 = useCallback((base64String: string): ValidationResult => {
        ...
        try {
            ...
            if (cleanString.startsWith('data:')) {
                ...
                if (parts.length !== 2) {
                    return {
                        isValid: false,
                        error: createProcessingError('MALFORMED_DATA_URL', 'Data URL malformada')
                    }
                }
                ...
                if (!header.includes('image/')) {
                    return {
                        isValid: false,
                        error: createProcessingError('NOT_IMAGE_DATA_URL', 'Data URL no es de imagen')
                    }
                }
            ...
            if (!base64Regex.test(base64WithoutPrefix)) {
                return {
                    isValid: false,
                    error: createProcessingError('INVALID_BASE64_CHARS', 'Caracteres inválidos en base64')
                }
            }
            ...
            if (decoded.length === 0) {
                return {
                    isValid: false,
                    error: createProcessingError('EMPTY_DECODED_BASE64', 'Base64 decodificado está vacío')
                }
            }
    ...
    const getImageInfoFromBase64 = useCallback(async (
        base64String: string, 
        index: number
    ): Promise<ImageInfo> => {
                ...
                img.onload = () => {
                    try {
                        ...
                        if (img.width < MIN_IMAGE_DIMENSION || img.height < MIN_IMAGE_DIMENSION) {
                            reject(new Error(`Imagen demasiado pequeña: ${img.width}x${img.height}px (mínimo: ${MIN_IMAGE_DIMENSION}px)`))
                            return
                        }
                        
                        if (img.width > MAX_IMAGE_DIMENSION || img.height > MAX_IMAGE_DIMENSION) {
                            reject(new Error(`Imagen demasiado grande: ${img.width}x${img.height}px (máximo: ${MAX_IMAGE_DIMENSION}px)`))
                            return
                        }
    ...
    const getImageInfo = useCallback(async (file: File): Promise<ImageInfo> => {
        const validation = validateFile(file)
        if (!validation.isValid) {
            throw new Error(validation.error!.message)
        }
                ...
                new Promise<ImageInfo>((resolve, reject) => {
                    ...
                    img.onload = () => {
                        try {
                            // Validar dimensiones
                            if (img.width < MIN_IMAGE_DIMENSION || img.height < MIN_IMAGE_DIMENSION) {
                                reject(new Error(`Imagen demasiado pequeña: ${img.width}x${img.height}px`))
                                return
                            }
                            
                            if (img.width > MAX_IMAGE_DIMENSION || img.height > MAX_IMAGE_DIMENSION) {
                                reject(new Error(`Imagen demasiado grande: ${img.width}x${img.height}px`))
                                return
                            }
    ...
    const evaluateWithSaaS = useCallback(async (
        ...
        if (!base64Validation.isValid) {
            return { diagnostic: `Error: ${base64Validation.error!.message}` }
        }
    ...
    const processImage = useCallback(async (file: File): Promise<LivenessResult> => {
    ...
    const evaluateImages = useCallback(async (
        ...
        if (!Array.isArray(files)) {
            toast.error('Error', { description: 'Lista de archivos inválida' })
            return
        }

        if (files.length === 0) {
            toast.error('Error', { description: 'No se proporcionaron archivos' })
            return
        }
        ...
        if (validFiles.length === 0) {
            toast.error('Error', { description: 'No se encontraron archivos de imagen válidos' })
            return
        }
            ...
            for (let i = 0; i < validFiles.length; i += MAX_CONCURRENT_EVALUATIONS) {
                ...
                try {
                    ...
                    if (validFiles.length > 3) {
                        const progress = Math.round((processingContextRef.current!.processedItems / validFiles.length) * 100)
    ...
    const evaluateBase64Images = useCallback(async (
        ...
        if (!Array.isArray(base64Strings)) {
            toast.error('Error', { description: 'Lista de base64 inválida' })
            return
        }

        if (base64Strings.length === 0) {
            toast.error('Error', { description: 'No se proporcionaron base64' })
            return
        }
        ...
        if (validBase64s.length === 0) {
            toast.error('Error', { description: 'No se encontraron base64 válidos' })
            return
        }
            ...
            for (let i = 0; i < validBase64s.length; i += MAX_CONCURRENT_EVALUATIONS) {
                ...
                try {
                    ...
                    if (validBase64s.length > 3) {
                        const progress = Math.round((processingContextRef.current!.processedItems / validBase64s.length) * 100)
                        toast.info(`Progreso: ${progress}%`, {
    ...
    return {
        ...
        validateBase64: useCallback((base64: string) => validateBase64(base64).isValid, [validateBase64]),
        supportedExtensions: VALID_IMAGE_EXTENSIONS as readonly string[],
        // Funciones de utilidad adicionales
        canProcess: !loading,
        processingProgress: processingContextRef.current 
            ? Math.round((processingContextRef.current.processedItems / processingContextRef.current.totalItems) * 100)
            : 0
    }
}